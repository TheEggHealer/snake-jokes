import { Box, Title, Text, Stack, Center, Image, Button, Group, Divider, useMantineTheme, Modal } from "@mantine/core"
...
function InspectJokeModal({ joke, created, onClose }: InspectJokeModalProps) {
  ...
  return (
    <>
          ...
          {joke.images.length > 0 ?
            <Carousel withIndicators={joke.images.length > 1} withControls={joke.images.length > 1} height={height}>
              {joke.images.map(image => (
                <Carousel.Slide className="image" key={image}>
      ...
      <Modal opened={confirmOpened} onClose={closeConfirm} title="Confirm Delete" centered>
        <Stack gap={15}>
          <Text>Are you sure you want to delete this joke? This action cannot be undone.</Text>
          <Group justify="flex-end" gap={10}>
